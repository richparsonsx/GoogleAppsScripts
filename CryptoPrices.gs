function test() {
  var tCryp = 'BTC';
  var tCurr = 'USD';
  var test = getPrice(tCryp,tCurr) * 1;
  var rTest = number_format(test,2);
  console.log({message: 'Exchange for ' + tCryp + ' in ' + tCurr + ' = ' + rtest });
}

function getPrice(strCry, strCurr) {
  var url = 'https://min-api.cryptocompare.com/data/price?fsym=' + strCry + '&tsyms=' + strCurr;
  var xml = UrlFetchApp.fetch(url).getContentText();
  var strValue = xml.substring(xml.indexOf(":")+1);
